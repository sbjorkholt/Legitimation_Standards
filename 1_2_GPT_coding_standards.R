...
fetch_responses(data = abstracts,
                ...
                version = "2",
                api_key = api_key,
                output_folder = "./Data/standards/")

completions <- lapply(list.files(paste0("./Data/standards/"), full.names = TRUE, pattern = ".txt"), read.table) %>%
  bind_rows()

tokenuse <- lapply(list.files(paste0("./Data/standards/"), full.names = TRUE, pattern = ".csv"), read.csv) %>%
  bind_rows()

ids <- str_remove(str_extract(list.files(paste0("./Data/standards/"), full.names = TRUE, pattern = ".txt"), "[0-9]+\\.txt"), ".txt")
